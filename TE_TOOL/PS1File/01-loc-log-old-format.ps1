...
foreach ($file in $log_files) {
    ...
    switch -regex ($fileName) {
        "^PASS.*600I" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "600I" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*25G" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "25G" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*DL" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "DL" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*PT0" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT0" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*PT1" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT1" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*PT2" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT2" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*PT3" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT3" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*PT4" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT4" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*PT" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*FT1" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT1" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*FT2" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT2" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*FT3" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT3" -passFolder $passFolder; $count_pass++; continue }
        "^PASS.*FT4" { join_and_move_pass -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT4" -passFolder $passFolder; $count_pass++; continue }
    ...
    # Phân loại FAIL
    switch -regex ($fileName) {
        "^FAIL.*600I" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "600I" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*25G" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "25G" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*DL" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "DL" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*PT0" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT0" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*PT1" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT1" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*PT2" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT2" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*PT3" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT3" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*PT4" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT4" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*PT" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "PT" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT1" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT1" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT2" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT2" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT3" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT3" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT4" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT4" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT5" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT5" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT6" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT6" -failFolder $failFolder; $count_fail++; continue }
        "^FAIL.*FT7" { join_and_move_fail -log_dir $final_LOG_FOLDER -file_name $fileName -state "FT7" -failFolder $failFolder; $count_fail++; continue }
    }
}
